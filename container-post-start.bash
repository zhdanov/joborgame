...
then
    ...
    git clone git@gitlab-$ENVIRONMENT.gitlab-$ENVIRONMENT:$HOME_USER_NAME/hangar.git /usr/local/composer-store/hangar
    pushd /usr/local/composer-store/hangar
        git archive --format zip --output ../hangar.zip master
    popd


    git clone git@gitlab-$ENVIRONMENT.gitlab-$ENVIRONMENT:$HOME_USER_NAME/samovar.git /usr/local/composer-store/samovar
